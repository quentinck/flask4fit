...
class ChinaGarminClient(object):
    ...
    def _authenticate(self):
        ...
        self.session.get('https://connect.garmin.cn/legacy/session')
    ...
    def _fetch_activity_ids_and_ts(self, start_index, max_limit=100):
        ...
        response = self.session.get(
            "https://connect.garmin.cn/modern/proxy/activitylist-service/activities/search/activities",
    ...
    def get_activity_summary(self, activity_id):
        ...
        :rtype: dict
        """
        response = self.session.get("https://connect.garmin.cn/modern/proxy/activity-service/activity/{}".format(activity_id))
        if response.status_code != 200:
    ...
    def get_activity_details(self, activity_id):
        ...
        response = self.session.get("https://connect.garmin.cn/modern/proxy/activity-service/activity/{}/details".format(activity_id))
        if response.status_code != 200:
    ...
    def get_activity_gpx(self, activity_id):
        ...
        :rtype: str
        """
        response = self.session.get("https://connect.garmin.cn/modern/proxy/download-service/export/gpx/activity/{}".format(activity_id))
    ...
    def get_activity_tcx(self, activity_id):
        ...
        response = self.session.get("https://connect.garmin.cn/modern/proxy/download-service/export/tcx/activity/{}".format(activity_id))
    ...
    def get_original_activity(self, activity_id):
        ...
        :rtype: (str, str)
        """
        response = self.session.get("https://connect.garmin.cn/modern/proxy/download-service/files/activity/{}".format(activity_id))
    ...
    def upload_activity(self, file, format=None, name=None, description=None, activity_type=None, private=None):
        ...
        response = self.session.post("https://connect.garmin.cn/modern/proxy/upload-service/upload/.{}".format(format),
        ...
        if data:
            ...
            response = self.session.put("https://connect.garmin.cn/proxy/activity-service/activity/{}".format(activity_id), data=json.dumps(data), headers=encoding_headers)
